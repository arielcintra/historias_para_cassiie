...
import { Grid, Button, Paper, Stack, Typography, Checkbox, FormControlLabel, Box } from "@mui/material";
...
export default function ExportPDF() {
  ...
  useEffect(() => {
    ...
    const handlePdfCollageUpdate = (event: CustomEvent) => {
      if (event.detail.bookId === activeBook?.id && event.detail.chapterId === chapterId) {
        refreshPreview();
      }
    };
    
    window.addEventListener('storage', handleStorageChange);
    window.addEventListener('pdfCollageUpdate', handlePdfCollageUpdate as EventListener);
    
    return () => {
      window.removeEventListener('storage', handleStorageChange);
      window.removeEventListener('pdfCollageUpdate', handlePdfCollageUpdate as EventListener);
    };
  ...
  const handlePrintMultiple = () => {
    ...
    const chaptersToExport = activeBook.chapters.filter(c => selectedChapters.includes(c.id));
    const w = window.open("", "_blank")!;
    w.document.write(renderMultipleChaptersHTML(activeBook.title, chaptersToExport, activeBook.id));
    w.document.close();
    w.focus();
    w.print();
  };
    ...
    <Grid container spacing={2}>
      <Grid size={{ xs: 12, md: 4 }}>
        <Stack spacing={2}>
          <Paper 
            sx={{ 
              p: 3,
              background: "linear-gradient(135deg, rgba(255,255,255,0.95), rgba(249,168,212,0.3))",
              borderRadius: 3,
                  ...
                  <FormControlLabel
                    ...
                    control={
                      <Checkbox
                        ...
                        onChange={(e) => handleChapterSelection(c.id, e.target.checked)}
                        size="small"
                      />
                    }
                    label={c.title}
            ...
            {activeBook.chapters
              ...
              .map((c) => (
                <Paper
                  ...
                  sx={{
                    p: 2.5,
                    border: c.id === chapterId
                      ? "2px solid #ec4899"
                      : "1px solid rgba(236, 72, 153, 0.2)",
                    background: c.id === chapterId
                ...
                >
                  ...
                  >
                    {(() => {
                      if ('pageNumber' in c) {
                        const pdfCollages = JSON.parse(localStorage.getItem('pdf-collages') || '{}');
                        const savedCollage = pdfCollages[`${activeBook.id}-${c.id}`];
                        return savedCollage ? "✨ Editado (PDF + stickers)" : "📄 Sem edição";
                      }
                      return c.collage ? "✨ Editado (história + stickers)" : "📝 Sem edição";
                    })()}
                  </Typography>
                </Paper>
              ))}
      ...
      <Grid size={{ xs: 12, md: 6 }}>
        <Paper 
          sx={{ 
            p: 3,
            background: "linear-gradient(135deg, rgba(255,255,255,0.95), rgba(243,232,255,0.8))",
            borderRadius: 3,
            border: "1px solid rgba(236, 72, 153, 0.2)",
            boxShadow: "0 8px 32px rgba(236, 72, 153, 0.15)"
          }}
        >
          ...
          {chapter && 'pageNumber' in chapter ? (
            <Paper key={refreshKey} sx={{ p: 2, height: 360, position: 'relative', overflow: 'hidden' }}>
              <PDFViewer 
    ...
    .map(
      (it) => {
        const emojiContent = it.emoji.startsWith("data:image/svg+xml") 
          ...
          : it.emoji;
        return `<div style="position:absolute;left:calc(${it.x * 100}% - 14px);top:calc(${
          it.y * 100
...
.page{min-height:100vh;padding:48px} 
...
</head><body><section class="page"><h1>${chapterTitle}</h1><div class="story"><img src="${imagePath}" alt="Página do livro" class="pdf-image"/>${items}</div></section></body></html>`;
}
...
function renderMultipleChaptersHTML(
  ...
  const pagesHTML = chapters.map((chapter) => {
    let collageToUse = chapter.collage;
    ...
    if ('pageNumber' in chapter) {
      ...
      return `<section class="page"><h1>${chapter.title}</h1><div class="story"><p>${escape(text)}</p>${items}</div></section>`;
    }
  }).join('');
